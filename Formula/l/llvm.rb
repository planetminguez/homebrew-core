class Llvm < Formula
  ...
  def install
    # Work around OOM error on arm64 linux runner by using half the number of jobs
    github_arm64_linux = OS.linux? &&
                         Hardware::CPU.arm? &&
                         ENV["HOMEBREW_GITHUB_ACTIONS"].present? &&
                         ENV["GITHUB_ACTIONS_HOMEBREW_SELF_HOSTED"].blank?
    if github_arm64_linux && (jobs = ENV.make_jobs / 2).positive?
